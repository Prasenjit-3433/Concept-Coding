...
**Binary Search on Answer:** Feasibility simulates a sequential process — iterate the array, greedily accumulate (days, loads, trips), check if the constraint is met. No placement decision. You are asking: *"given this capacity/speed, can I finish the task?"*

**Minimax / Maximin:** Feasibility involves a placement decision with a gap or load constraint. You are asking: *"can I place all elements such that no two are closer than mid?"* The greedy inside is about **where to place**, not how much to accumulate.

**The one-line test:**
> Feasibility iterates and **accumulates** → Pattern 4 (Binary Search on Answer).
> Feasibility iterates and **places / assigns** → Pattern 5 (Minimax).

---

## How to tell Binary Search on Answer apart from Minimax in an unseen problem

Both look identical from outside — binary search on a number, check if it works. The difference is what "check if it works" means.

**Binary Search on Answer:** You are given a capacity/speed/limit. You simulate: walk through the array, keep adding, reset when you exceed the limit, count how many resets you needed. Pure accumulation. No decision about *where* something goes.

**Minimax / Maximin:** You are placing items into positions. You walk through positions and ask "is this gap large enough to place here?" You count how many items you successfully placed. The check is about *placement*, not accumulation.

**One-line test:**
> Does your feasibility function **count resets**? → Pattern 4.
> Does your feasibility function **count placements**? → Pattern 5.
...
**Partition:** You pick a candidate split index in the smaller array. You derive the split index in the second array. You check four boundary values. No counting at all. Only works when you have exactly two sorted arrays and need a single order statistic (usually median).

**One-line test:**
> Do you have one or more sorted structures and need the kth value? → Pattern 6 (Count ≤ Mid).
...
These two alone solve LC 34, LC 35, LC 744, and a large fraction of sorted array problems.
...
**Identify:** The array is not uniformly sorted — it is rotated, has a peak, or is a mountain. Standard binary search breaks because the "discard half" logic relies on uniform order globally. The key insight in each variant: find a **local structural property** that still lets you confidently discard one half.
...
**Identify:** A 2D matrix with sorted structure. Three distinct techniques depending on the structure — identify which applies before writing a single line.
...
- **Row and column sorted independently** (LC 240): binary search does not apply cleanly. Use staircase search from top-right — go left if too large, go down if too small. O(m+n).
...
**Identify:** You are not searching for a value in an array. You are searching for the **optimal value of the answer itself** — minimum speed, minimum capacity, fewest days. The answer space is a range of integers. The feasibility check **simulates a sequential process** — iterate the array, greedily accumulate, verify the constraint. No placement decision involved.
...
**Identify:** The problem asks to **minimize the maximum** or **maximize the minimum** value after placing or assigning elements under a gap or load constraint. The feasibility check makes placement decisions — greedily place each element and check if the required count fits within the constraint.
...
- The feasibility function asks "can I place all N elements such that every consecutive gap is ≥ mid?" — a placement question, not an accumulation question
- The search space is often inverted vs Pattern 4: you search for the **largest feasible minimum** (upper bound template) rather than the smallest feasible maximum
...
| Kth Element — Partition | 2 | Binary search on split index, boundary comparison |
...
**Patterns 4 and 5 are the highest interview ROI.** Together they cover the majority of binary search questions at product companies. Use the one-line test from the intro section every time you see a new problem.

**Pattern 6 before Pattern 7 without exception.** Count ≤ Mid builds the intuition that binary search can operate on a value domain. LC 4's partition approach will feel approachable only after that foundation is solid.
